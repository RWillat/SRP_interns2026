...
# ---------------------------------------------------------------------------
# Replication (approximate, pin-cell scale) of SerpentTests/HPMR.sss
#
# Scope of this first pass: a single triangular-pitch "flower" unit cell
# (1 heat pipe pin surrounded by a ring of 6 alternating fuel/moderator pins,
# pitch = 2.3 cm as in the Serpent `lat 20` card) with reflective boundaries,
# for a k-inf style check. The full core (19x19 assembly lattice, 13x13
# assembly-of-assemblies, 12 control drums, axial Be reflectors) is not
# modeled here. TRISO particles are re-packed stochastically at PF=40% with
# openmc.model.pack_spheres rather than reusing the exact 614k-particle
# Serpent coordinate file (per instruction, since the original placement was
# itself stochastic).
# ---------------------------------------------------------------------------
...
# --- TRISO layers (Serpent `particle 900` card, radii in cm) ---
...
# Graphite matrix inside the compact (background between TRISO particles)
...
# Graphite matrix outside the compact (fills the rest of each pin cell)
...
# Heat pipe working fluid (Na vapor/liquid + wick), homogenized as in Serpent
...
# materials.export_to_xml() is deferred to the end of the script, once the
# fuel kernel volume (needed for the specific-power postprocessing) is known.

## Geometry ##

pitch = 2.3     # cm, triangular pin pitch (Serpent `lat 20`)
h_pin = 4.0     # cm, axial slice height of the reflective unit cell
pf = 0.40       # TRISO packing fraction (PF40 case)

# TRISO layer radii, cm (Serpent `particle 900` card)
r_kernel = 0.02125
r_buffer = 0.03125
r_PyC1 = 0.03525
r_SiC = 0.03875
...
r_compact = 1.00    # surf 2, fuel compact radius
r_mod_core = 0.825  # surf 3
r_mod_shell = 0.92  # surf 3g
r_hp_core = 0.97    # surf 51
r_hp_shell = 1.07   # surf 5g

z_lo = openmc.ZPlane(z0=-h_pin / 2, boundary_type='reflective')
z_hi = openmc.ZPlane(z0=h_pin / 2, boundary_type='reflective')

# --- TRISO particle (fuel/buffer/PyC1/SiC/PyC2 concentric spheres) ---
...
# Pack spheres into a region shrunk by r_PyC2 on every side so that no
# TRISO particle's outer surface extends past the compact/lattice box
# (otherwise OpenMC warns that a particle lies outside the lattice).
...
# Exact fuel kernel volume in this unit cell, for the specific-power
# (fission energy per loaded uranium mass) postprocessing metric.
n_kernels = len(triso_centers)
mats['fuel'].volume = n_kernels * (4.0 / 3.0) * np.pi * r_kernel ** 3
...
# --- Fuel compact pin (universe "3" in Serpent) ---
...
# --- Moderator pin (universe "2" in Serpent) ---
...
# --- Heat pipe pin (universe "1" in Serpent) ---
...
# --- fuel/moderator pins, triangular pitch (approximates the local mix ---
# --- of pin types seen in the Serpent `lat 20` assembly pattern).      ---
num_rings = 2  # center ring + 1 ring of neighbors
...
## Plot (for visually checking the pin layout / hex tiling before running) ##
plot = openmc.Plot()
...
# Fission rate in the fuel kernel material
...
# Flux energy spectra in key regions, using the 11-group structure from
# Serpent's `set nfg` card (converted MeV -> eV, with 1e-5 eV / 20 MeV
# added as the overall lower/upper bounds)
...
# Spatial flux distribution over the unit cell footprint
